...
class RegisterSecretaryDialog extends StatefulWidget {
  ...
  @override
  _RegisterSecretaryDialogState createState() => _RegisterSecretaryDialogState();
}
...
class _RegisterSecretaryDialogState extends State<RegisterSecretaryDialog> {
  ...
  @override
  void dispose() {
    for (final c in [_nameC, _emailC, _passwordC, _phoneC, _birthdayC]) {
      c.dispose();
    }
    super.dispose();
  }
        ...
        BlocConsumer<RegisterSecretaryCubit, RegisterSecretaryState>(
          listener: (context, state) {
            if (state is RegisterSecretarySuccess) {
              ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(state.result.message)));
